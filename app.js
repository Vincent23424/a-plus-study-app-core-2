/* CompTIA A+ Core 2 Study App - application logic */
const pbqScenarios = [
    {
        title: "System File Corruption Incident",
        desc: "A Windows 11 client system is experiencing system lockups. You suspect corrupted system files.",
        objective: "Run the correct command line utility in elevated privilege mode to scan and automatically repair corrupted protected Windows system files.",
        hint: "Use the System File Checker utility with the /scannow switch.",
        expected: ["sfc /scannow"],
        success: "Windows Resource Protection found corrupt files and successfully repaired them.",
        failure: "Incorrect command. Try running 'sfc /scannow'."
    },
    {
        title: "Network Connectivity Failure",
        desc: "A user lost connection to local resources and cannot access external sites after a IP change.",
        objective: "Release and renew the IPv4 address configuration via Command Prompt.",
        hint: "Use ipconfig with /release and /renew.",
        expected: ["ipconfig /renew", "ipconfig /release"],
        success: "IP address successfully renewed. Network connectivity restored.",
        failure: "Check your ipconfig switches."
    },
    {
        title: "SOHO WPA3 Migration",
        desc: "Upgrade security for a wireless network to modern standards.",
        objective: "Configure WPA3 Personal with SAE in the prompt simulator.",
        hint: "Type 'wpa3' or 'sae' to apply settings.",
        expected: ["wpa3", "sae"],
        success: "Wireless security successfully upgraded to WPA3-Personal (SAE).",
        failure: "Command not recognized for this scenario."
    }
];
...
class MainApp {
    ...
    updateReadiness() {
        ...
        Object.values(this.state.stats).forEach(s => {
            correct += s.c || 0;
            attempted += (s.c || 0) + (s.w || 0);
        });
        const coverage = DATA.questions.length ? Math.min(Object.keys(this.state.answered).length, DATA.questions.length) / DATA.questions.length : 0;
        ...
        if (v) v.textContent = readiness + '%'; 
        if (b) b.style.width = readiness + '%';
    }

    startAllModules() {
        const sel = document.getElementById('all-module-count'); 
        const val = sel ? sel.value : '20';
        const pool = [...DATA.questions].sort(() => Math.random() - .5); 
        const n = val === 'all' ? pool.length : Math.min(Number(val), pool.length);
        this.startSession(pool.slice(0, n), `All Modules — ${n} Questions`);
    }

    changeFlashcardCount(v) {
        this.flashcardCount = v === 'all' ? 'all' : Number(v);
    ...
    populateFlashcardDropdown() {
        ...
        DATA.modules.forEach(m => {
            ...
            o.value = m.id;
            o.textContent = `Module ${m.id}: ${m.title}`;
            s.appendChild(o);
        });
        s.value = 'all';
        this.selectedModuleId = 'all';
    ...
    changeFlashcardModule(v) {
        this.selectedModuleId = v === 'all' ? 'all' : Number(v);
        this.loadFlashcards(this.selectedModuleId);
    }

    startFlashcardsForModule(v) {
    ...
    loadFlashcards(v) {
        ...
        const n = this.flashcardCount || 20; 
        this.currentCards = [...all].sort(() => Math.random() - .5).slice(0, n === 'all' ? all.length : Math.min(n, all.length));
        this.cardIndex = 0;
        this.isCardFlipped = false;
        const m = DATA.modules.find(x => x.id === Number(v));
        
        const badge = document.getElementById('flashcard-module-badge');
        if (badge) badge.textContent = v === 'all' ? 'All Modules' : `Module ${m ? m.id : v}`;
        
        const title = document.getElementById('flashcard-title');
        if (title) title.textContent = v === 'all' ? 'Core 2 — Modules 11–22' : (m ? m.title : '');
        
        const av = document.getElementById('flashcards-available');
        if (av) av.textContent = `${all.length} available`;
        
        this.renderCard();
    }

    populatePBQDropdown() {
        const existing = document.getElementById('pbq-scenario-select');
        if (!existing) return;
    ...
    submitPBQ() {
        ...
        const ok = sc.expected.some(x => cmd === x.toLowerCase() || cmd.includes(x.toLowerCase())); 
        const fb = document.getElementById('pbq-feedback');
        if (fb) {
            fb.className = `mb-3 p-3 rounded-xl text-xs border ${ok ? 'bg-emerald-950/40 border-emerald-800 text-emerald-300' : 'bg-rose-950/40 border-rose-800 text-rose-300'}`;
            fb.innerHTML = ok ? `<b>✓ Correct</b><div class="mt-1">${sc.success}</div>` : `<b>✗ Not correct</b><div class="mt-1">${sc.failure}</div><div class="mt-1 text-slate-500">Use the hint if needed and submit again.</div>`;
        }
        if (ok) {
    ...
    renderReference() {
        const data = {
            "Ports & Protocols": [["22","SSH — Secure Shell"],["23","Telnet"],["25","SMTP"],["53","DNS"],["67/68","DHCP"],["80","HTTP"],["110","POP3"],["143","IMAP"],["137–139","NetBIOS/NetBT"],["389","LDAP"],["443","HTTPS"],["445","SMB/CIFS"],["3389","RDP"],["5900","VNC"]],
            ...
            "Script & File Types": [[".ps1","PowerShell script"],[".bat","Batch file"],[".vbs","VBScript"],[".js","JavaScript"],[".py","Python"],[".pyw","Python windowed script"],[".exe","Windows executable"],[".msi","Windows installer package"]],
            "Windows Commands & Tools": [["ipconfig","Windows IP configuration"],["ping","Network connectivity test"],["netstat","Network connection/status information"],["nslookup","DNS query tool"],["tracert","Route tracing tool"],["pathping","Network path and packet-loss information"],["chkdsk","Disk/file-system checking tool"],["diskpart","Disk partition management tool"],["robocopy","File copying tool"],["gpupdate","Updates Group Policy settings"],["gpresult","Displays resulting Group Policy settings"],["sfc","System File Checker"],["eventvwr.msc","Event Viewer"],["diskmgmt.msc","Disk Management"],["taskschd.msc","Task Scheduler"],["devmgmt.msc","Device Manager"],["certmgr.msc","Certificate Manager"],["lusrmgr.msc","Local Users and Groups"],["perfmon.msc","Performance Monitor"],["gpedit.msc","Group Policy Editor"],["msinfo32.exe","System Information"],["resmon.exe","Resource Monitor"],["msconfig.exe","System Configuration"],["cleanmgr.exe","Disk Cleanup"],["dfrgui.exe","Disk Defragment"],["regedit.exe","Registry Editor"]],
            "Wireless / Connectivity": [["2.4 GHz","Wi-Fi frequency band"],["5 GHz","Wi-Fi frequency band"],["6 GHz","Wi-Fi frequency band"],["802.11","IEEE wireless LAN standard"],["NFC","Near Field Communication"],["RFID","Radio Frequency Identification"]],
            "Backup & Operations": [["Full backup","Copies all selected data"],["Incremental backup","Copies changes since the last backup"],["Differential backup","Copies changes since the last full backup"],["GFS","Grandfather-Father-Son backup rotation"],["3-2-1","Backup rule described in Core 2 objectives"],["CAB","Change board / change review"],["Risk analysis","Evaluation of risk associated with a change"]]
        }; 
        const root = document.getElementById('reference-content');
        if (!root) return;
        root.innerHTML = Object.entries(data).map(([title, items]) => `
            <div class="glass-card rounded-2xl p-5 border border-slate-800">
                <h3 class="font-bold text-white mb-3">${title}</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                    ${items.map(x => `<div class="p-3 rounded-xl bg-slate-900 border border-slate-800"><div class="font-mono font-bold text-cyan-300 text-xs">${x[0]}</div><div class="text-xs text-slate-300 mt-1">${x[1]}</div></div>`).join('')}
                </div>
            </div>
        `).join('');
    }
    ...
    updateGlobalStats() {
        ...
        if (statTotal) statTotal.textContent = totalQ;
        const statAcc = document.getElementById('stat-accuracy-home');
        if (statAcc) statAcc.textContent = `${accuracy}%`;
        const statWrong = document.getElementById('stat-wrong-home');
        if (statWrong) statWrong.textContent = this.state.wrong.length;
        const statStarred = document.getElementById('stat-starred-home');
        if (statStarred) statStarred.textContent = this.state.starred.length;

        // Feature card badges
        const badgeWrong = document.getElementById('badge-wrong');
        if (badgeWrong) badgeWrong.textContent = this.state.wrong.length;
        const badgeWeak = document.getElementById('badge-weak');
        if (badgeWeak) badgeWeak.textContent = weakCount;
        const badgeStarred = document.getElementById('badge-starred');
        if (badgeStarred) badgeStarred.textContent = this.state.starred.length;
        const badgeNew = document.getElementById('badge-new');
        if (badgeNew) badgeNew.textContent = DATA.questions.filter(q => !this.state.answered[q.id]).length;
    }
    ...
    startSession(pool, label) {
        ...
        const countVal = countSelect ? countSelect.value : '20';
    ...
    renderCard() {
        ...
        if (back) back.textContent = card.def;
        const count = document.getElementById('card-progress-count');
        if (count) count.textContent = `${this.cardIndex + 1} / ${this.currentCards.length}`;
    }
...
// GitHub Pages navigation history + back button.
(function(){
    const originalNavigateTo = MainApp.prototype.navigateTo;
    MainApp.prototype._viewHistory = ['home'];
    
    MainApp.prototype.goBack = function(){
        if(!this._viewHistory) this._viewHistory=['home'];
        if(this._viewHistory.length > 1){
            this._viewHistory.pop();
            const previous = this._viewHistory[this._viewHistory.length - 1] || 'home';
            this._navigatingBack = true;
            originalNavigateTo.call(this, previous);
            this._navigatingBack = false;
        } else {
            originalNavigateTo.call(this, 'home');
        }
        updateBackButton(this);
    };

    MainApp.prototype.navigateTo = function(viewId){
        if(!this._viewHistory) this._viewHistory=['home'];
        const current = this.currentView || this._viewHistory[this._viewHistory.length-1] || 'home';
        if(!this._navigatingBack && viewId !== current){
            this._viewHistory.push(viewId);
            if(this._viewHistory.length > 20) this._viewHistory.shift();
        }
        originalNavigateTo.call(this, viewId);
        updateBackButton(this);
    };

    function updateBackButton(instance){
        const row = document.getElementById('view-back-row');
        if(row) row.classList.toggle('hidden', !instance || instance._viewHistory.length <= 1 || instance.currentView==='home');
    }

    window.addEventListener('load', function(){
        window.app = new MainApp();
        app._viewHistory = ['home'];
        updateBackButton(app);
    });
})();
